  ...
  - [理论依据](#理论依据)
    ...
    - [Reed-Solomon 编码](#reed-solomon-编码)
      ...
      - [编码矩阵](#编码矩阵)
        ...
        - [柯西编解码过程优化](#柯西编解码过程优化)
    - [分离数据与控制链接](#分离数据与控制链接)
      - [上传/下载速度的提升](#上传下载速度的提升)
        - [传统 C/S 模式](#传统-cs-模式)
        - [集中式对等网络](#集中式对等网络)
  - [技术依据](#技术依据)
    - [Docker 部署服务端](#docker-部署服务端)
    - [实现多用户权限支持的技术](#实现多用户权限支持的技术)
    ...
    - [纠删码的实现](#纠删码的实现)
      - [现有的开源项目](#现有的开源项目)
      - [应用 WebAssembly](#应用-webassembly)
      - [DOM API](#dom-api)
    - [Token 实现身份验证](#token-实现身份验证)
      - [Token 概述](#token-概述)
      - [JWT 标准](#jwt-标准)
      - [非对称加密](#非对称加密)
      - [潜在风险](#潜在风险)
    - [OpenVPN 建立虚拟局域网](#openvpn-建立虚拟局域网)
  - [技术路线](#技术路线)
    - [可移植性和易用性](#可移植性和易用性)
    - [扩展至多人使用](#扩展至多人使用)
    - [减轻中央服务器的负载](#减轻中央服务器的负载)
    - [采用工具](#采用工具)
    - [应用设想](#应用设想)
      - [小目标](#小目标)
      - [大目标](#大目标)
...
Signature 里有三个部分，先是用 Base64 编码的 Header 和 Payload，再用加密算法加密。加密的时候放入一个 Secret ，这是一个秘密地存储在服务端的密码。
