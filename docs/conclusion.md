...
在上述两种技术中，IPFS 本身在去中心化上做的很充分，同时，因为完全去除了中心化的元素，也有没有解决的问题：

1. 没有解决存储数据的安全问题：没有验证用户身份，任何人只要拿到文件的哈希值就能拿到文件。

2. 没有解决数据可靠性问题：IPFS系统虽然有激励层吗，但是无法保证一份数据能够有**足够的备份**在系统中存储，这就可能导致数据丢失

我们的项目不仅有IPFS的去中心化的优势，还能避免其两大问题。这都归功于设计中的目录节点，我们之后会详细介绍。

![image-20200720182915233](conclusion.assets/image-20200720182915233.png)

至于NAS的局限性在于使用专有设备，前期安装和设备成本较高。NAS的可扩展性也有一定的限制，因为单台 NAS 的扩容是有限的，增加另一台NAS设备非常容易，但是要想将两个NAS设备的存储空间*无缝合并*就不容易了。 

而在我们的项目中，普通设备只需要运行一个客户端即可成为存储节点，（后面还会介绍客户端是支持跨平台的），并且我们的可扩展性上不存在NAS的问题。

#### 前人工作

我们还参考了 2017年 OSH 课程中的一个大作业项目，也是一个[基于互联网的分布式文件系统](https://github.com/IngramWang/DFS_OSH2017_USTC)。但是该项目的数据传输是中心化的，所有文件都要经过中央服务器的中转，有很多不利的地方。
...
例如这个时候可以加入预约离线下载。当前我们的项目下载文件时只能收集瞬时在线的碎片，但是进行离线下载的话，目录节点可以帮我在一段连续的时间内收集碎片，这样一来成功率会进一步提升，并且受设备在线时间的影响被降低了。

还有可能某些需要访问文件的设备配置较低，设备编解码效率低下，这个时候可以要求目录节点代为执行编解码。
